...
static const char *TAG = "ota_tcp";

#define OTA_TCP_MAGIC 0x45544F41u /* ETOA */
#define OTA_TCP_VERSION 1u
#define OTA_TCP_TASK_STACK 8192
#define OTA_TCP_CHUNK_SIZE 4096
...
static esp_err_t validate_header(const ota_tcp_header_t *header, uint32_t *image_size)
{
    ...
    ESP_RETURN_ON_FALSE(magic == OTA_TCP_MAGIC, ESP_ERR_INVALID_ARG, TAG, "bad magic");
    ESP_RETURN_ON_FALSE(version == OTA_TCP_VERSION, ESP_ERR_INVALID_VERSION, TAG, "bad protocol version");
    ESP_RETURN_ON_FALSE(header_len == sizeof(ota_tcp_header_t), ESP_ERR_INVALID_SIZE, TAG, "bad header length");
    ...
    ESP_RETURN_ON_FALSE(size <= partition->size, ESP_ERR_INVALID_SIZE, TAG, "image too large for OTA partition");
...
#if CONFIG_OTA_TCP_REQUIRE_VERSION_INCREMENT
...
static esp_err_t receive_image(int sock, const ota_tcp_header_t *header)
{
    ...
    uint8_t *buffer = malloc(OTA_TCP_CHUNK_SIZE);
    if (buffer == NULL) {
    ...
    while (remaining > 0) {
        const size_t want = remaining > OTA_TCP_CHUNK_SIZE ? OTA_TCP_CHUNK_SIZE : remaining;
    ...
    ESP_LOGI(TAG, "OTA complete; rebooting in %d ms", CONFIG_OTA_TCP_REBOOT_DELAY_MS);
    return ESP_OK;
}

static void ota_tcp_server_task(void *arg)
{
    int listen_sock = -1;
    ...
    while (true) {
        ...
        struct sockaddr_in addr = {
            .sin_family = AF_INET,
            .sin_port = htons(CONFIG_OTA_TCP_PORT),
        ...
        ESP_LOGI(TAG, "listening on TCP port %d", CONFIG_OTA_TCP_PORT);
        while (true) {
            ...
            if (err == ESP_OK) {
                ...
                close(sock);
                vTaskDelay(pdMS_TO_TICKS(CONFIG_OTA_TCP_REBOOT_DELAY_MS));
                esp_restart();
            } else {
...
esp_err_t ota_tcp_server_start(void)
{
    BaseType_t ok = xTaskCreate(ota_tcp_server_task, "ota_tcp_server", OTA_TCP_TASK_STACK, NULL, 5, NULL);
